...
public class DigitRecognizer implements AutoCloseable {
  private Session sess;
  private final int imgSize;
  private Tensor keepProb;
  protected float[] inputSpace;
  protected static Logger logger = LogManager.getLogger(DigitRecognizer.class);

  public DigitRecognizer(String modelDir) {
    this(modelDir, "serve", 28);
  }

  public DigitRecognizer(String modelDir, String tags, int imgSize) {
  ...
  public static void main(String[] args) {
    ...
    try (DigitRecognizer reco = new DigitRecognizer("mnist", "serve", 28)) {
  ...
  public int[] predict(
          Iterable<byte[]> imgBatch, boolean toBinary, int batchSize, int[] predictions) {
  ...
  public float[][] proba(
          Iterable<byte[]> imgBatch, boolean toBinary, int batchSize, float[][] dst) {
